OC.L10N.register(
    "files_mindmap",
    {
    "You cannot open a folder" : "Huwezi kufungua folda",
    "This file is too big to be opened. Please download the file instead." : "This file is too big to be opened. Please download the file instead.",
    "Cannot read the file." : "Haiwezi kusoma faili",
    "Invalid file path supplied." : "Invalid file path supplied.",
    "The file is locked." : "Faili hii imefungwa",
    "An internal server error occurred." : "Hitilafu ya ndani ya seva imetokea",
    "You cannot write to a folder" : "Huwezi kuandika kwenye folda",
    "The file you are working on was updated in the meantime. You cannot save your progress as saving would overwrite these changes. Please reload the page." : "Faili unayofanyia kazi ilisasishwa kwa sasa. Huwezi kuhifadhi maendeleo yako kwani kuhifadhi kunaweza kubatilisha mabadiliko haya. Tafadhali pakia upya ukurasa.",
    "Could not write to file." : "Haikuweza kuandika kwenye faili.",
    "Insufficient permissions" : "Ruhusa haitoshi",
    "File path not supplied" : "Njia ya faili haijatolewa",
    "File mtime not supplied" : "Faili mtime haijatolewa",
    "Share not found" : "Ushirikishaji haupo",
    "You are not authorized to open this share" : "Hujaidhinishwa kufungua kushiriki huku",
    "File not found" : "Faili halipatikani",
    "You have no permission to write this file" : "Huna ruhusa ya kuandika faili hii",
    "Mind Map" : "Ramani ya Akili",
    "A mind map editor" : "Mhariri wa ramani ya mawazo",
    "This application enables Nextcloud users to open, save and edit mind map files in the web browser. If enabled, an entry in the \"New\" button at the top of the web browser the mind map file entry appears. When clicked, a new mind map file opens in the browser and the file can be saved into the current Nextcloud folder." : "Programu hii inawawezesha watumiaji wa Nextcloud kufungua, kuhifadhi na kuhariri faili za ramani ya mawazo katika kivinjari cha wavuti. Ikiwashwa, ingizo katika kitufe cha \"Mpya\" kilicho juu ya kivinjari cha wavuti ingizo la faili ya ramani ya mawazo linaonekana. Unapobofya, faili mpya ya ramani ya mawazo hufungua kwenye kivinjari na faili inaweza kuhifadhiwa kwenye folda ya sasa ya Nextcloud.",
    "Does not support saving {extension} files." : "Haitumii kuhifadhi {extension} faili.",
    "File Saved" : "Faili imehifadhiwa",
    "Save failed" : "Uhifadhi umeshindikana",
    "Unsupported file type: {mimetype}" : "Aina ya faili isiyotumika: {mimetype}",
    "Edit" : "Hariri",
    "New mind map file" : "Faili mpya ya ramani ya mawazo",
    "New mind map" : "Ramani mpya ya mawazo",
    "AutoSave" : "Hifadhi Kiotomatiki",
    "Save" : "Hifadhi",
    "Export" : "Safirisha",
    "Export to PNG" : "Hamisha kwa PNG",
    "Export to SVG" : "Hamisha kwa SVG",
    "Export to PDF" : "Hamisha kwa PDF",
    "Export to Markdown" : "Hamisha kwa Markdown",
    "Export to Text" : "Hamisha kwa maandishi",
    "Saved as {name}" : "Imehifadhiwa kama {name}",
    "\"{name}\" was created — your changes are saved there. The original .mm file is unchanged." : "\"{name}\" imeundwa — mabadiliko yako yamehifadhiwa hapo. Faili la asili .mm halijabadilika.",
    "\"{name}\" already exists. Overwrite it with the content from \"{source}\"?" : "\"{name}\" tayari lipo. Je, liandike upya na maudhui kutoka \"{source}\"?",
    "\"{name}\" already exists. Please choose a different name." : "\"{name}\" tayari lipo. Tafadhali chagua jina tofauti.",
    "Enter a new filename (without extension):" : "Ingiza jina jipya la faili (bila kipande):",
    "Conversion cancelled" : "Ubadilishaji umeghairiwa"
},
"nplurals=2; plural=(n != 1);");
